  ...
  return (
    <div className="flex h-full flex-1 flex-col gap-3 md:flex-row md:items-center">
      ...
      <ul className="flex max-h-[220px] flex-col gap-1.5 overflow-y-auto md:w-1/2">
        {data.map((datum) => (
          <li key={datum.key} className="text-sm flex items-center gap-2">
            <span className="size-2.5 flex-shrink-0 rounded-full" style={{ backgroundColor: colorByKey[datum.key] }} />
            <span
              className="min-w-0 flex-1 truncate text-secondary"
              title={formatName ? formatName(datum) : datum.name}
            >
