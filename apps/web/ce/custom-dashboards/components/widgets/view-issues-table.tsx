...
const IssueRow = observer(function IssueRow(props: { issue: TViewListIssue }) {
  ...
  return (
    <tr className="border-b border-subtle last:border-b-0">
      <td className="text-xs py-2 pr-3 whitespace-nowrap text-tertiary">
        {identifier ? `${identifier}-${issue.sequence_id}` : `#${issue.sequence_id}`}
      </td>
      <td className="max-w-0 py-2 pr-3">
        <span className="text-sm block truncate text-primary" title={issue.name}>
          {issue.name}
        </span>
      </td>
...
export const ViewIssuesTableWidget = observer(function ViewIssuesTableWidget(props: Props) {
  ...
  const { data, error, isLoading, mutate } = useSWR(["dashboard-widget-issues", widget.id, cursor, perPage], () =>
        ...
        <div className="flex flex-1 flex-col">
          <div className="flex-1 overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-subtle text-left text-11 tracking-wide text-tertiary uppercase">
          ...
          {envelope && (envelope.prev_page_results || envelope.next_page_results) && (
            <div className="mt-3 flex items-center justify-end gap-2">
              <button
                ...
                className={cn(
                  "text-xs flex items-center gap-1 rounded border border-subtle px-2 py-1 text-secondary hover:bg-layer-2",
