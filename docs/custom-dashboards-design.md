...
db_table values. Reusing any of `dashboards`, `widgets`, `dashboard_widgets`,
or the `deprecated_*` table names for this feature would either collide
outright or make a future upstream diff on those tables ambiguous about which
feature owns them. `fork_workspace_dashboard_widgets` is unambiguous and can
