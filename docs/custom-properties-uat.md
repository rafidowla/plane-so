...
| Item            | Expected                                                                                                                                                                         |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Instance flag   | On by default — no env var needed. Only set `CUSTOM_PROPERTIES_ENABLED=0` on the **api** (and worker) container env if you want to opt this instance out                         |
...
> Kill switch: set `CUSTOM_PROPERTIES_ENABLED=0` and restart the api → the
> entire feature disappears (see §6). This is the rollback lever. (Prior to
> 2026-07-26 the flag defaulted to off and had to be explicitly set to `1`;
> it's now on by default, so unsetting it no longer disables anything.)
...
| 1.2 | With the instance flag **off** (`CUSTOM_PROPERTIES_ENABLED=0`) | Toggle is **disabled** and greyed, with the notice "Custom properties are turned off for this deployment…" |
...
| 4.6 | _(known gap)_ Create a **brand-new** work item via the modal          | The Status input is **not** shown during create (set it right after creating). This is the documented v1 limitation |
...
Set `CUSTOM_PROPERTIES_ENABLED=0` on the api/worker and restart (the flag is
on by default, so unsetting it does nothing — it must be set to `0`
explicitly). All endpoints return "disabled", all seams render nothing, and
stock Plane behaviour is restored. Data is retained (soft-deleted rows stay);
